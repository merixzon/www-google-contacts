...
my $google1 = WWW::Google::Contacts->new(username => 'foo', password => 'foo');
my $google2 = WWW::Google::Contacts->new(username => 'bar', password => 'bar');

my $contact1 = $google1->new_contact({ phone_number => '123456' });
is( $contact1->server->username, 'foo', 'Contact1 - Server username is correct');
is( ref $contact1->phone_number, 'ARRAY', 'Contact1 - phone_number is an array');
...
is( $contact2->server->username, 'bar', 'Contact2 - Server username is correct');
...
is( $contact3->server->username, 'bar', 'Contact3 - Server username is correct');
